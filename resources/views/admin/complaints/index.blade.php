<x-app-layout>
    <x-slot name="header">
        {{-- هنا أضفنا div لتنظيم العنوان والزر --}}
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('User Complaints (Admin Panel)') }}
            </h2>
            {{-- زر "Add New Complaint Type" --}}
            <a href="{{ route('admin.complaint_types.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm ">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('All User Complaints') }}</h3>
                    ...
                    <form method="GET" action="{{ route('admin.complaints.index') }}" class="mb-6 bg-gray-50 p-4 rounded-lg shadow-inner">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <x-input-label for="complaint_type_id" :value="__('Filter by Complaint Type')" />
                        ...
                        <div class="mt-4 text-right flex justify-end space-x-2">
                            ...
                            {{-- زر إعادة تعيين الفلاتر يظهر فقط إذا كانت هناك فلاتر مطبقة --}}
                            @if(request()->filled('user_id') || request()->filled('complaint_type_id'))
                    ...
                    @if ($complaints->isEmpty())
                        <p class="text-center text-gray-600">{{ __('No complaints found matching your criteria.') }}</p>
                    @else
                                    ...
                                    @foreach ($complaints as $complaint)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <a href="{{ route('admin.complaints.show', $complaint->id) }}" class="text-indigo-600 hover:text-indigo-900">
                                                    {{ __('View/Manage') }}
                                                </a>
                                            </td>
                                        </tr>
